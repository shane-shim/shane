...
def send_discord_notification(message, status="info"):
    ...
    embed = {
        "title": "💬 Airflow Q&A봇 모니터링",
        "description": message,
        "color": colors.get(status, 0x0080ff),
        "timestamp": datetime.utcnow().isoformat(),
        "footer": {"text": "Airflow Q&A Bot Monitor"}
    }
...
def check_qna_bot_log(**context):
    """Q&A봇 로그 확인"""
    # SSH로 원격 서버의 로그 확인
    cmd = [
        "ssh", "-i", SSH_KEY, f"{SSH_USER}@{SSH_HOST}",
        "tail -50 /home/nerdlab-datastudio-python/adaptive_qna_cron.log | grep -E '(완료|Error|error)'"
    ]
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # 로그에서 성공/실패 확인
            if "적응형 Q&A 크롤링 완료" in log_content:
                # 최근 실행 시간 확인
                time_cmd = [
                    "ssh", "-i", SSH_KEY, f"{SSH_USER}@{SSH_HOST}",
                    "stat -c %Y /home/nerdlab-datastudio-python/adaptive_qna_cron.log"
                ]
                ...
                if time_result.returncode == 0:
                    ...
                    if time_diff < 2:  # 2시간 이내
                        print("Q&A봇이 정상적으로 실행되었습니다.")
                        return "success"
                    else:
                        print(f"Q&A봇이 {time_diff:.1f}시간 전에 마지막으로 실행되었습니다.")
                        return "warning"
            
            elif "Error" in log_content or "error" in log_content:
                print("Q&A봇 실행 중 오류 발생")
...
def run_qna_bot_manually(**context):
    """Q&A봇 수동 실행"""
    cmd = [
        "ssh", "-i", SSH_KEY, f"{SSH_USER}@{SSH_HOST}",
    ...
    try:
        ...
        if result.returncode == 0:
            send_discord_notification(
                "✅ Q&A봇이 Airflow에 의해 성공적으로 재실행되었습니다.",
                status="success"
            )
            return True
        else:
...
def check_and_notify(**context):
    """상태 확인 및 Discord 알림"""
    status = context['task_instance'].xcom_pull(task_ids='check_qna_bot_log')
    
    if status == "error":
        error_log = context['task_instance'].xcom_pull(task_ids='check_qna_bot_log', key='error_log')
        send_discord_notification(
            f"⚠️ Q&A봇 오류 감지\\n```{error_log}```\\n재실행을 시도합니다...",
            status="warning"
        )
        ...
        send_discord_notification(
            "⚠️ Q&A봇이 2시간 이상 실행되지 않았습니다. 재실행을 시도합니다...",
            status="warning"
        )
        return "needs_restart"
    elif status == "unknown":
        send_discord_notification(
            "❓ Q&A봇 상태를 확인할 수 없습니다. 관리자 확인이 필요합니다.",
            status="warning"
        )
        return "unknown"
    else:
...
default_args = {
    ...
    'start_date': datetime(2025, 1, 14, 18, 0),
...
dag = DAG(
    ...
    description='Q&A봇 모니터링 및 자동 재실행',
    schedule_interval='10 18 * * *',  # 매일 오후 6시 10분
    catchup=False,
    tags=['monitoring', 'qna_bot'],
)

# Task 1: Q&A봇 로그 확인
check_log_task = PythonOperator(
    task_id='check_qna_bot_log',
    python_callable=check_qna_bot_log,
    dag=dag,
)
...
restart_task = PythonOperator(
    task_id='restart_qna_bot',
    python_callable=run_qna_bot_manually,
...
# 매시간 실행되는 상태 체크 DAG
hourly_dag = DAG(
    'qna_bot_hourly_check',
    default_args=default_args,
    description='Q&A봇 시간별 상태 체크',
    schedule_interval='30 * * * *',  # 매시간 30분
    catchup=False,
    tags=['monitoring', 'qna_bot', 'hourly'],
)
...
def hourly_health_check(**context):
    ...
    cmd = [
        "ssh", "-i", SSH_KEY, f"{SSH_USER}@{SSH_HOST}",
        "find /home/nerdlab-datastudio-python/adaptive_qna_cron.log -mmin -1440 | wc -l"
    ]
